#!/usr/bin/env python3
"""
GhostTalk Bot v5.5 - FULLY FIXED & COMPLETE
- Fixed frozen bug on /next and /stop
- Fixed report TypeError
- Added emojis everywhere
- Admin-only features (no match notifications to users)
...
BANNED_WORDS = ['fuck', 'fucking', 'sex chat', 'nudes', 'pussy', 'dick', 'cock', 'penis', 'vagina', 'boobs', 'tits', 'ass', 'asshole', 'bitch', 'slut', 'whore', 'hoe', 'prostitute', 'porn', 'pornography', 'rape', 'child', 'pedo']
LINK_PATTERN = re.compile(r'https?://|www\.', re.IGNORECASE)
BANNED_PATTERNS = [re.compile(re.escape(w), re.IGNORECASE) for w in BANNED_WORDS]

COUNTRIES = {
    'india': '🇮🇳', 'united states': '🇺🇸', 'pakistan': '🇵🇰', 'nepal': '🇳🇵', 'uk': '🇬🇧',
    'usa': '🇺🇸', 'us': '🇺🇸', 'canada': '🇨🇦', 'australia': '🇦🇺', 'germany': '🇩🇪',
    'france': '🇫🇷', 'japan': '🇯🇵', 'china': '🇨🇳', 'russia': '🇷🇺', 'brazil': '🇧🇷',
    'mexico': '🇲🇽', 'bangladesh': '🇧🇩', 'sri lanka': '🇱🇰', 'indonesia': '🇮🇩',
    'philippines': '🇵🇭', 'thailand': '🇹🇭', 'vietnam': '🇻🇳', 'south korea': '🇰🇷',
    'north korea': '🇰🇵', 'turkey': '🇹🇷', 'egypt': '🇪🇬', 'south africa': '🇿🇦',
    'singapore': '🇸🇬', 'malaysia': '🇲🇾', 'uae': '🇦🇪', 'saudi arabia': '🇸🇦',
    'new zealand': '🇳🇿', 'ireland': '🇮🇪', 'italy': '🇮🇹', 'spain': '🇪🇸',
    'netherlands': '🇳🇱', 'sweden': '🇸🇪', 'norway': '🇳🇴', 'switzerland': '🇨🇭',
    'argentina': '🇦🇷', 'chile': '🇨🇱', 'colombia': '🇨🇴', 'peru': '🇵🇪',
    'venezuela': '🇻🇪', 'greece': '🇬🇷', 'portugal': '🇵🇹', 'czech': '🇨🇿',
    'poland': '🇵🇱', 'hungary': '🇭🇺', 'romania': '🇷🇴', 'ukraine': '🇺🇦',
    'finland': '🇫🇮', 'denmark': '🇩🇰', 'austria': '🇦🇹', 'belgium': '🇧🇪',
    'israel': '🇮🇱', 'kenya': '🇰🇪', 'nigeria': '🇳🇬', 'morocco': '🇲🇦',
...
def db_create_user_if_missing(user):
    ...
    with get_conn() as conn:
        conn.execute(
            ...
            (uid, user.username or '', user.first_name or '', None, None, None, None, datetime.now(datetime.UTC if hasattr(datetime, 'UTC') else None).isoformat(), ref_code)
        )
        conn.commit()
...
def db_is_premium(user_id):
    ...
    if not u or not u.get('premium_until'):
        return False
    try:
        return datetime.fromisoformat(u.get('premium_until')) > datetime.now()
    except:
        return False
...
def db_is_banned(user_id):
    ...
    with get_conn() as conn:
        ...
        if ban_until:
            try:
                return datetime.fromisoformat(ban_until) > datetime.now()
            except:
                return False
        return False
    ...
    with get_conn() as conn:
        if permanent:
            conn.execute(
                ...
                (user_id, None, 1, reason, ADMIN_ID, datetime.now().isoformat())
            )
        else:
            until = (datetime.now() + timedelta(hours=hours)).isoformat() if hours else None
            conn.execute(
                'INSERT OR REPLACE INTO bans (user_id, ban_until, permanent, reason, banned_by, banned_at) VALUES (?, ?, ?, ?, ?, ?)',
                (user_id, until, 0, reason, ADMIN_ID, datetime.now().isoformat())
            )
        conn.commit()
...
def db_add_report(reporter_id, reporter_name, reported_id, reported_name, report_type, reason, chat_status):
    with get_conn() as conn:
        ...
        if report_count >= AUTO_BAN_REPORTS:
            ...
            conn.execute(
                'INSERT OR REPLACE INTO bans (user_id, ban_until, permanent, reason, banned_by, banned_at) VALUES (?, ?, ?, ?, ?, ?)',
                (reported_id, ban_until, 0, f'Auto-ban {report_count} reports', ADMIN_ID, datetime.now().isoformat())
            )
...
def db_save_recent_partner(user_id, partner_id):
    with get_conn() as conn:
        ...
        reconnect_until = (datetime.now() + timedelta(minutes=5)).isoformat()
        conn.execute(
...
def db_get_recent_partner(user_id):
    with get_conn() as conn:
        ...
        try:
            if datetime.fromisoformat(reconnect_until) > datetime.now():
                return partner_id
        except:
            pass
...
def warn_user(user_id, reason):
    with user_warnings_lock:
        ...
        if count >= WARNING_LIMIT:
            ...
            try:
                bot.send_message(user_id, f'⚠️ You have been temporarily banned for {TEMP_BAN_HOURS} hours.')
            except:
                pass
...
def disconnect_user(user_id):
    ...
    try:
        ...
        markup.add(types.InlineKeyboardButton('📋 Report', callback_data='report_end'))
...
def announce_match(u1, u2):
    ...
    match_markup.add(
        types.InlineKeyboardButton('⏭️ Next', callback_data=f'match_nxt_{u1}'),
        types.InlineKeyboardButton('🛑 Stop', callback_data=f'match_end_{u1}')
    )
...
def cmd_start(message):
    ...
    if not u or not u.get('gender'):
        ...
        markup.add(
            types.InlineKeyboardButton('👨 Male', callback_data='sex_m'),
            types.InlineKeyboardButton('👩 Female', callback_data='sex_f')
        )
...
/search_opposite - 💎 Find opposite gender
/stop - 🛑 Exit current chat
/next - ⏭️ Find new partner
...
2️⃣ No adult content or spam
3️⃣ Protect your privacy
4️⃣ Share media only with consent
5️⃣ No harassment or abuse
...
👥 Referred: {u.get('referral_count', 0)}/3'''
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton('🎂 Age', callback_data='age_ch'),
        types.InlineKeyboardButton('👨 Male', callback_data='sex_m'),
        types.InlineKeyboardButton('👩 Female', callback_data='sex_f'),
        types.InlineKeyboardButton('🌍 Country', callback_data='cnt_ch')
    )
    
    bot.send_message(uid, settings_text, reply_markup=markup)

@bot.callback_query_handler(func=lambda c: c.data in ['age_ch', 'cnt_ch'])
...
def cmd_search_random(message):
    ...
    with active_pairs_lock:
        if uid in active_pairs:
            bot.send_message(uid, '⚠️ You are already chatting. Use /next or /stop')
            return
    ...
    with queue_lock:
        ...
        search_start_time[uid] = datetime.now()
...
def cmd_search_opposite(message):
    ...
    with active_pairs_lock:
        if uid in active_pairs:
            bot.send_message(uid, '⚠️ You are already chatting.')
            return
...
# ============================================================================
# REPORT SYSTEM - FIXED TYPO ERROR
# ============================================================================
def report_keyboard():
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
...
def cmd_report(message):
    ...
    bot.send_message(uid, '📋 Why are you reporting this user? Select a reason:', reply_markup=report_keyboard())
...
def handle_report_buttons(call):
    ...
    if data.startswith('rep_'):
        ...
        with active_pairs_lock:
            is_active = (uid in active_pairs and active_pairs[uid] == reported_id)
...
# ============================================================================
# MATCH CALLBACKS - FIXED FROZEN BUG
# ============================================================================
...
def cmd_admin(message):
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton('📋 All Reports', callback_data='adm_rep'),
        types.InlineKeyboardButton('👥 All Users', callback_data='adm_usr'),
        types.InlineKeyboardButton('🚫 Ban User', callback_data='adm_ban'),
        types.InlineKeyboardButton('❌ Permanent Ban', callback_data='adm_pbm'),
        types.InlineKeyboardButton('💬 Active Chats', callback_data='adm_act'),
        types.InlineKeyboardButton('💎 Approve Premium', callback_data='adm_pre'),
        types.InlineKeyboardButton('📊 Bot Stats', callback_data='adm_sta')
    )
...
def callback_admin_panel(call):
    ...
    if data == 'adm_rep':
        ...
        with get_conn() as conn:
            users = conn.execute('SELECT user_id, first_name, gender, age, country, messages_sent FROM users ORDER BY user_id DESC LIMIT 15').fetchall()
        
        user_text = f'👥 USERS (Latest 15):\n\n'
        ...
        bot.send_message(uid, '🚫 Enter user ID to ban (24 hours):')
        ...
        bot.send_message(uid, '💎 Enter user ID to make premium (1 year):')
        bot.register_next_step_handler(call.message, process_admin_premium)
    
    elif data == 'adm_act':
        with active_pairs_lock:
            total_pairs = len(active_pairs) // 2
            active_list = list(active_pairs.items())[:10]
        
        if not active_list:
            bot.send_message(uid, '💬 No active chats.')
        else:
            active_text = f'💬 ACTIVE CHATS: {total_pairs}\n\n'
            for user_id, partner in active_list:
                u = db_get_user(user_id)
                p = db_get_user(partner)
                u_name = u.get('first_name', 'User') if u else 'User'
                p_name = p.get('first_name', 'User') if p else 'User'
                active_text += f"👤 {u_name} ↔️ {p_name}\n"
        ...
        with get_conn() as conn:
            ...
            banned_count = conn.execute('SELECT COUNT(*) FROM bans').fetchone()[0]
...
def process_ban_user(message):
    ...
    try:
        ban_user_id = int(message.text or '')
...
def process_admin_premium(message):
    ...
    try:
        premium_user_id = int(message.text or '')
        u = db_get_user(premium_user_id)
        if u:
            until_date = (datetime.now() + timedelta(days=365)).isoformat()
...
def cmd_reconnect(message):
        ...
        if uid in reconnect_cooldown:
            ...
            try:
                if datetime.fromisoformat(cooldown_until) > datetime.now():
                    remaining = datetime.fromisoformat(cooldown_until) - datetime.now()
    ...
    with active_pairs_lock:
        if uid in active_pairs:
            bot.send_message(uid, '⚠️ You are chatting. Use /stop first.')
            return
    ...
    with active_pairs_lock:
        if partner_id in active_pairs:
            bot.send_message(uid, '⚠️ Your partner is busy.')
            return
    ...
    reconnect_markup.add(
        types.InlineKeyboardButton('✅ Accept', callback_data=f'recon_yes_{uid}'),
        types.InlineKeyboardButton('❌ Decline', callback_data=f'recon_no_{uid}')
    )
...
def handle_reconnect_response(call):
    ...
    with reconnect_lock:
        ...
        if (datetime.now() - req_time).seconds > RECONNECT_TIMEOUT:
    ...
    if action == 'yes':
        with active_pairs_lock:
            if requester_id in active_pairs or partner_id in active_pairs:
                bot.answer_callback_query(call.id, '⚠️ Someone is busy', show_alert=True)
                try:
                    bot.send_message(requester_id, '⚠️ Your partner is busy.')
                except:
                    pass
                return
        ...
        cooldown_until = (datetime.now() + timedelta(hours=RECONNECT_COOLDOWN_HOURS)).isoformat()
        with reconnect_lock:
...
def forward_chat_message(message):
    ...
    if is_banned_content(text):
        warn_user(uid, '⚠️ Violated community rules')
        return
...
def handle_media_message(message):
    ...
    try:
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton('✅ Accept', callback_data=f'med_yes_{uid}'),
            types.InlineKeyboardButton('❌ Decline', callback_data=f'med_no_{uid}')
        )
...
if __name__ == '__main__':
    ...
    logger.info('=' * 70)
    logger.info('🤖 GhostTalk Bot v5.5 - COMPLETE & FULLY FIXED')
    logger.info('=' * 70)
    logger.info('✅ Anonymous Random Chat')
    logger.info('✅ Full Chat Forwarding')
    logger.info('✅ Media Accept/Reject')
    logger.info('✅ Report System (5+ reasons)')
    logger.info('✅ Admin Panel (Reports, Users, Ban, Stats)')
    logger.info('✅ Ban System (Temp & Permanent)')
    logger.info('✅ Reconnect System (5 min + 24h cooldown)')
    logger.info('✅ Premium System (3 referrals)')
    logger.info('✅ Content Filter + Warnings')
    logger.info('✅ All Emojis Added')
    logger.info('✅ Fixed /next & /stop Frozen Bug')
    logger.info('✅ Fixed Report TypeError')
    logger.info('=' * 70)
