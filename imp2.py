...
active_pairs = {}
user_warnings = {}
pending_media = {}  # sender_id -> {partner_id, media_type, file_id}
...
def handle_media(m):
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("✅ Accept", callback_data=f"app:{uid}:{media_type}"),
        types.InlineKeyboardButton("❌ Reject", callback_data=f"rej:{uid}:{media_type}")
    )
    ...
    try:
        if media_type == 'photo':
            bot.send_photo(partner, media_id, caption=caption, reply_markup=markup)
        elif media_type == 'document':
            bot.send_document(partner, media_id, caption=caption, reply_markup=markup)
        elif media_type == 'video':
            bot.send_video(partner, media_id, caption=caption, reply_markup=markup)
        elif media_type == 'animation':
            bot.send_animation(partner, media_id, caption=caption, reply_markup=markup)
        elif media_type == 'sticker':
            bot.send_sticker(partner, media_id, reply_markup=markup)
    except Exception as e:
...
def approve_media_cb(call):
    try:
        ...
        media_type = parts[2] if len(parts) > 2 else "media"
